	...
	<div class="row">
		<div class="col-sm-4">
			<?php
				$id = intval(@$_GET['id']);
				if($id > 0 && $db = get_db()) {
					$query = "SELECT `badges`.`name`, `badge_categories`.`category_name`, `badge_difficulties`.`difficulty_name`, `badges`.`description`, `badges`.`instructions`  FROM `badges` INNER JOIN `badge_difficulties` ON `badges`.`difficulty`=`badge_difficulties`.`id` INNER JOIN `badge_categories` ON `badges`.`category`=`badge_categories`.`id` WHERE `badges`.`id`='$id' AND `badges`.`is_disabled`='0';";
					if ($result = $db->query($query)) {
						if ($row = $result->fetch_assoc()) {
							//print_r($row);
							echo "<h1>".ucwords(strtolower($row['name']))."</h1>";
							echo "<h3>Category: ".ucwords(strtolower($row['category_name']))."</h3>";
							echo "<h3>Difficulty: ".ucfirst(strtolower($row['difficulty_name']))."</h3>";
