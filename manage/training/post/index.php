...
<?php	
	...
	if ($post_id == 0 && (canManageAllBlogPosts() || canAddBlogPost())) {
		if ($db) {
			...
			$query = "INSERT INTO `training_posts` (`created_by`, `publish_time`) VALUES ('$user_id', '$now');";
			if ($db->query($query)) {
	...
	$query = "SELECT * FROM `training_posts` WHERE `id`='$post_id';";
...
?>
	<div class="row">
		...
		<div class="col-md-4">
			<a href="/training/post?id=<?php echo $post_id;?>"><button class="btn btn-primary">View Post</button></a>
		</div>
	</div>
